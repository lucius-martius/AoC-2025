...
;; Day 8 - Part 1
(with-temp-buffer
  (insert-file-contents-literally "input/day8")
  (goto-char (point-min))
  (let ((count 0)
        (connmax 1000)
        (boxes nil)
        ...
        (networks nil))
    (while (not (= (point) (point-max)))
      (when (looking-at (rx (group (+ num)) ?, (group (+ num)) ?, (group (+ num))))
        (push (list (string-to-number (match-string 1))
                    (string-to-number (match-string 2))
                    (string-to-number (match-string 3)))
              boxes))
      (forward-line))
    (seq-do-indexed (lambda (from idx)
                      (dolist (to (nthcdr (1+ idx) boxes))
                        (push (list (+ (expt (- (car to) (car from)) 2)
                                       (expt (- (cadr to) (cadr from)) 2)
                                       (expt (- (caddr to) (caddr from)) 2))
                                    from to)
                              distances)))
                    boxes)
    (let ((l (seq-sort-by 'car '< distances)))
      (dotimes (_ connmax)
        (let* ((a (cadr (car l)))
               (b (caddr (car l)))
               (ain (seq-find (lambda (network) (member a network)) networks))
               (bin (seq-find (lambda (network) (member b network)) networks))
               (either (xor ain bin))
               (other (if ain b a)))
          (cond ((or (not l) (and ain bin (equal ain bin))))
                ((and ain bin)
                 (setq networks (delete bin networks))
                 (setcdr (last ain) bin))
                (either
                 (setcdr either (cons other (cdr either))))
                (t
                 (push (list a b) networks))))
        (setq l (cdr l))))
    (let ((networks (seq-sort-by 'length '> networks)))
      (* (length (car networks))
         (length (cadr networks))
         (length (caddr networks))))))

;; Day 8 - Part 2
;; This was the only day where the part 2 was actually much easier and a simpler program in
;; total. It turned out, the limited number of connections made the program more complex.
(with-temp-buffer
  (insert-file-contents-literally "input/day8")
  (goto-char (point-min))
  (let ((count 0)
    ...
    (seq-do-indexed (lambda (from idx)
                      (dolist (to (nthcdr (1+ idx) boxes))
                        (push (cons (+ (expt (- (car to) (car from)) 2)
                                       (expt (- (cadr to) (cadr from)) 2)
                                       (expt (- (caddr to) (caddr from)) 2))
                                    (cons from to))
                              distances)))
                    boxes)
    (let (lastconn)
      (dolist (dist (seq-sort-by 'car '< distances))
        (let* ((a (cadr dist))
               (b (cddr dist))
               (ain (seq-find (lambda (network) (member a network)) networks))
               (bin (seq-find (lambda (network) (member b network)) networks))
               (either (xor ain bin))
               (other (if ain b a)))
          (cond ((and ain bin (equal ain bin)))
                ((and ain bin)
                 (setq networks (delete bin networks))
                 (setcdr (last ain) bin)
                 (setq lastconn (cons a b)))
                (either
                 (setcdr either (cons other (cdr either)))
                 (setq lastconn (cons a b)))
                (t
